...
unsigned int  sky_power[19] =  {5800,1200, 1200,1200, 550,550, 550,550, 550,1200, 550,550, 550,550, 550,1200, 550,1200, 550};  // UNKNOWN A80E7E5E
...
unsigned int  num0[11] = {550,1250, 500,650, 550,1200, 550,1200, 1150,650, 500};  // UNKNOWN F32F72D7


  /*
unsigned int  num1[11] = {500,650, 550,650, 500,1250, 500,650, 550,1200, 550};  // UNKNOWN C9767F76
unsigned int  num2[11] = {550,600, 550,650, 1100,650, 500,650, 1100,650, 550};  // UNKNOWN C8155AB1
unsigned int  num3[11] = {550,600, 550,600, 1150,1250, 500,650, 1100,1250, 550};  // UNKNOWN B6996DAE
unsigned int  num4[11] = {550,600, 550,1250, 550,650, 500,600, 1150,1200, 550};  // UNKNOWN 969AE844
unsigned int  num5[11] = {500,650, 500,1250, 550,1200, 550,1200, 550,650, 500};  // UNKNOWN 4AAFAC67
unsigned int  num6[11] = {550,600, 550,1250, 1100,650, 550,1200, 550,1200, 550};  // UNKNOWN 9C2A936C
unsigned int  num7[11] = {500,650, 550,1250, 1050,1250, 550,1200, 1100,700, 500};  // UNKNOWN 833ED333
unsigned int  num8[11] = {500,650, 1100,650, 500,650, 550,1200, 1150,650, 500};  // UNKNOWN 55F2B93
unsigned int  num9[11] = {500,650, 1100,650, 550,1200, 550,1200, 1150,1200, 550};  // UNKNOWN DE78B0D0
unsigned int  numd[11] = {550,1200, 550,650, 1100,650, 550,1200, 1150,1200, 550};  // UNKNOWN BDE97C12
unsigned int  nume[11] = {500,1250, 500,650, 1150,1200, 1150,600, 550,650, 500};  // UNKNOWN 3F23F43
...
for (int i=0;i<8;i++){
 ...
 codeA_size = sizeof(codeA) / sizeof(codeA[0]);       
  ...
  if(Serial.available()){
    ...
    switch(command){
      ...
      case 20:
       ...
       delay(40);
        //irsend.sendRaw(aoc_power, sizeof(aoc_power) / sizeof(aoc_power[0]), khz); //Note the approach used to automatically calculate the size of the array.
      break;      
      case 21:
     ...
     /*
      ...
      break;
      case 25:
       irsend.sendRaw(aoc_source, sizeof(aoc_voldown) / sizeof(aoc_voldown[0]), 38);
      break;
      case 26:
       irsend.sendRaw(aoc_mute, sizeof(aoc_voldown) / sizeof(aoc_voldown[0]), 38);
      break;
       */    
      case 30:
...
/*
    ...
    */
      ...
      case 0:
       ...
       // irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(codeA, codeA_size, khz); //Note the approach used to automatically calculate the size of the array.
      break;
      case 1:
      //  irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num1, sizeof(num0) / sizeof(num0[0]), khz); //Note the approach used to automatically calculate the size of the array.
      break;
      case 2:
     //   irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num2, sizeof(num0) / sizeof(num0[0]), khz); //Note the approach used to automatically calculate the size of the array.
      break;
      case 3:
    //    irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num3, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 4:
   //     irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num4,sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 5:
   //     irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num5, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 6:
      //  irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num6, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 7:
     //   irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num7, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 8:
     //   irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num8, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 9:
     //   irsend.sendRaw(headA, sizeof(headA) / sizeof(headA[0]), khz);
        irsend.sendRaw(num9, sizeof(num0) / sizeof(num0[0]), khz);
      break;
      case 10:
